...
def create_spark_session(spark_master):
    spark=SparkSession.builder\
            .appName("StocksDataTransformation")\
            .master(spark_master)\
...
# %%
def read_kafka_stock_stream(spark,kafka_server,kafka_topic):
...
# %%
def parse_stock_json_data(df):
    """Parse JSON stock data from Kafka"""
    stock_schema=define_stock_schema()

    json_df=df.selectExpr("CAST(value AS STRING) as json_str")

    parsed_df=json_df.select(from_json(col("json_str"), stock_schema).alias("data")).select("data.*")

    return parsed_df

# %%
def apply_stock_transformations(df):
    """Apply business logic transformations to stock data"""
    logger.info("Applying transformations to stock data")
...
# %%
def apply_stock_quality_checks(df):
    """Apply data quality checks to stock data"""
    logger.info("Applying data quality checks to stock data")
    return df\
...
# %%
def process_stock_batch(df, epoch_id, output_path):
    """Process each batch of stock data - IMPROVED VERSION"""
    try:
        logger.info(f"Processing stock batch {epoch_id}")
        ...
        transformed_df = apply_stock_transformations(df)
        quality_checked_df = apply_stock_quality_checks(transformed_df)
        ...
        logger.error(f"Error processing stock batch {epoch_id}: {str(e)}")
        import traceback
        traceback.print_exc()
...
def foreach_batch_function(df, epoch_id, output_path):
    ...
    if df.count() > 0:
        process_stock_batch(df, epoch_id, output_path)
...
def main():
    ...
    try:
        ...
        kafka_df = read_kafka_stock_stream(spark, args.kafka_servers, args.kafka_topic)

        logger.info(f"Kafka stream started from topic {args.kafka_topic} on servers {args.kafka_servers}")
        
        # Parse JSON data
        parsed_df = parse_stock_json_data(kafka_df)
        ...
        logger.info(f"Stock transformation job started successfully")
        ...
        logger.error(f"Error in Stock transformation job: {str(e)}")
